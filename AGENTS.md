...
This file is the non-Claude-harness compatibility layer for the `.claude/` system (opencode, Gemini CLI, Codex).

Keep this file aligned with [CLAUDE.md](CLAUDE.md) as much as possible while adapting
Claude-native concepts to each host's native constructs.

Surfaces installed in this repo:

- `.claude/skills/` is the canonical source for shared skills and command-style workflows.
  opencode discovers `.claude/skills/` natively; Codex reads them through `.agents/skills/`,
  which is a symlink to `.claude/skills/` — new skills appear in both automatically.
- `.claude/agents/` is the canonical source for specialist agents and RIPER-5 mode agents.
- `.codex/agents/*.toml` mirrors `.claude/agents/` for Codex subagent roles.
- `.claude/hooks/` + `.claude/settings.json` are Claude-Code-only. opencode and Codex ignore them.
- NOT installed: `.codex/hooks/`, `.codex/config.toml`, and any `.opencode/` surface. Claude Code is
  the only fully wired host today. See `process/context/all-context.md` for the install record.

Prefer updating `.claude/` directly, then mirror to other hosts when needed.

Project context: read `GEMINI.md` in the repo root for authoritative architecture, modules,
dependency versions, conventions, and build/run instructions.
